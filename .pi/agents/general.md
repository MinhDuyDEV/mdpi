...
- **Read before editing or writing** — edit/write tools reject changes to existing files without a prior read
...
1. Read relevant files (prefer `semantic_inspect <symbol>` for fast symbol lookup)
...
**Code navigation:** Use pi-srcwalk for AST-aware search — `semantic_query` / `semantic_inspect` are preferred over grep for symbol definitions.
