> **Source:** Adapted from [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) (MIT).
...
**On your coding agent, use the built-in `Explore` subagent** rather than defining a custom research persona. `Explore` runs on a cheaper model tier, is denied write/edit tools, and is purpose-built for this pattern. Define a custom research subagent only when `Explore` doesn't fit (e.g. you need a domain-specific system prompt the model wouldn't infer).

---

## Coding agent compatibility

This catalog is harness-agnostic, but most readers will run it on a coding agent. Here's how each pattern maps onto a coding agent's primitives — and where the platform enforces our rules for us.
...
A coding agent may offer two parallelism primitives. Pattern 3 (parallel fan-out with merge) maps to **subagents**. If you need teammates that talk to each other, use **Agent Teams** instead.
...
| Status | Stable | Experimental — requires an opt-in flag |
...
Two rules in this catalog aren't just convention — a coding agent enforces them:

- **"Subagents cannot spawn other subagents"** (verbatim from the docs). Anti-pattern B (persona-calls-persona) and Anti-pattern D (deep persona trees) cannot exist by construction.
...
The fields that DO work in plugin agents are: `name`, `description`, `tools`, `disallowedTools`, `model`, `maxTurns`, `skills`, `memory`, `background`, `effort`, `isolation`, `color`, `initialPrompt`. Use `model` per-persona if you want to optimize cost (e.g. a cheaper tier for `test-engineer` coverage scans, a mid tier for `code-reviewer`, a premium tier for `security-auditor`).

### Spawning multiple subagents in parallel

In a coding agent, parallel fan-out (Pattern 3) requires issuing **multiple Agent tool calls in a single assistant turn**. Sequential turns serialize execution. `/ship` calls this out explicitly. Any new orchestrator command should do the same.
...
Agent Teams is experimental. In your agent settings, enable the experimental teams flag (for Claude Code, set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in `~/.claude/settings.json`).
...
3. The shared task list shows who's investigating what — visible at any time with the team status view.
...
You can interrupt any teammate at any time — useful for redirecting an investigator who's gone down a wrong path.
