---
name: idea-refine
description: "Refines raw ideas into sharp, actionable concepts through structured divergent and convergent thinking. Use when an idea is still vague, need to stress-test assumptions before committing to a plan, or want to expand options before converging on one. Triggers on 'ideate', 'refine this idea', 'stress-test my plan', 'brainstorm options'. Risk: Low."
---
...
- **`interview-me`**: upstream. If the ask is still underspecified ("build me X" without who/why), hand off to `interview-me` first to extract the real intent.
- **`brainstorming`**: parallel. Both are ideation-phase skills. `brainstorming` is collaborative questioning; `idea-refine` is structured output (one-pager + Not Doing list). Load both when exploring complex ideas.
- **`spec-driven-development`**: downstream. Once the idea is refined and a direction is chosen, hand off to `spec-driven-development` to write the specification.
- **`grill-me`**: complementary. After ideation, `grill-me` stress-tests the chosen direction adversarially.
