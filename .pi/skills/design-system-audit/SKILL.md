...
- Save audits to `.pi/artifacts/audits/`
- Save extracted token snapshots to `.pi/artifacts/tokens/`
...
|---|---|
| "The design system looks consistent to me" | Subjective consistency ≠ measured consistency. Audit with tools, not eyes. |
| "Small token deviations don't matter" | Token drift compounds. Each deviation erodes the system's value. |
| "We'll document patterns after the audit" | The audit IS the documentation. Find gaps, fix them, document the fix. |

## Red Flags

- Audit based on visual inspection instead of token comparison
- Inconsistent spacing values found but not quantified
- Component variants not catalogued before audit
- No distinction between intentional variation and accidental drift
- Audit findings without prioritization or remediation estimates

## Verification

After completing design system audit:

- [ ] All tokens (colors, spacing, typography) are catalogued and compared
- [ ] Deviations from the system are quantified and prioritized
- [ ] Component variants are consistent across all instances
- [ ] Audit findings include file:line references and fix recommendations
- [ ] A remediation plan with priorities is documented
