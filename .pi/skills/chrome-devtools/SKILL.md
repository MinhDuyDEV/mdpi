...
|---|---|
| "The console looks clean" | Clean console ≠ correct behavior. Inspect DOM state, network requests, and paint. |
| "I'll just use the browser manually" | Programmatic DevTools access provides reproducible evidence. Manual testing is not enough. |
| "Performance looks fine on my machine" | DevTools throttling exists for a reason. Test on target hardware profiles. |

## Red Flags

- Console errors ignored as "probably fine"
- Network requests returning 4xx/5xx not investigated
- Performance traces not captured before optimization
- Memory snapshots not taken when investigating leaks
- DevTools used only for console, ignoring Network, Performance, and Application tabs

## Verification

After using Chrome DevTools:

- [ ] Console errors are resolved or documented with justification
- [ ] Network tab shows no failed requests (4xx/5xx investigated)
- [ ] Performance trace captured for user-facing flows
- [ ] Memory snapshot analyzed if investigating leaks
- [ ] Findings are documented with screenshots and action items
