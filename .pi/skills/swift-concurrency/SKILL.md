...
|---|---|
| "I'll add @MainActor later" | Missing @MainActor annotations cause data races that are hard to reproduce and debug. |
| "The closure captures self, it's fine" | Strong reference cycles from closures are the #1 source of memory leaks in Swift. Use [weak self]. |
| "async/await is just syntactic sugar" | Structured concurrency has different semantics than GCD. Task hierarchies, cancellation, and priority matter. |
| "Sendable warnings are just compiler noise" | Sendable violations are data races waiting to happen. Fix every warning before it becomes a crash. |

## Red Flags

- @MainActor missing on UI-bound state
- Task {} created without cancellation handling
- Strong reference cycles in async closures
- Sendable conformance forced with @unchecked without understanding why
- async functions called without await (fire-and-forget without error handling)

## Verification

After writing Swift concurrency code:

- [ ] @MainActor is applied to all UI-bound state and functions
- [ ] Task cancellations are handled (checkTaskCancellation or try Task.checkCancellation())
- [ ] No strong reference cycles in async closures ([weak self] where needed)
- [ ] Sendable conformance is explicit (not @unchecked without documented reason)
- [ ] Data race warnings (TSan / Swift 6 language mode) are resolved, not suppressed
