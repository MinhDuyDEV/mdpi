...
|---|---|
| "It works locally, it'll work on Cloudflare" | Cloudflare Workers have different runtime constraints (CPU time, memory, subrequest limits). |
| "I'll hardcode the config for now" | Hardcoded config becomes stale config. Use environment variables and secrets. |
| "The free tier is enough for launch" | Free tier limits are real. Plan for scale before you need it. |
| "I'll add caching later" | Caching strategy should be part of the initial architecture, not an afterthought. |

## Red Flags

- Workers deployed without testing in wrangler dev first
- Hardcoded secrets in wrangler.toml or source code
- No error handling for Worker CPU timeouts
- KV/D1/R2 operations without error handling for eventual consistency
- Caching strategy not defined before launch

## Verification

After deploying to Cloudflare:

- [ ] `wrangler dev` testing passed for all endpoints
- [ ] Environment variables and secrets are configured in dashboard
- [ ] Error handling covers Worker-specific failures (CPU timeout, subrequest limit)
- [ ] Caching strategy is documented and tested
- [ ] Production deployment verified with actual traffic
