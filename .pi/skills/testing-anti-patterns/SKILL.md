...
```typescript
// [x] GOOD: Test real component or don't mock it
test('renders sidebar', () => {
...
```typescript
// [x] GOOD: Test utilities handle test cleanup
...
```typescript
// [x] GOOD: Mock at correct level
test("detects duplicate server", () => {
  // Mock the slow part, preserve behavior test needs
  vi.mock("MCPServerManager"); // Just mock slow server startup

  await addServer(config); // Config written
  await addServer(config); // Duplicate detected [x]
});
```
...
```typescript
// [x] GOOD: Mirror real API completeness
const mockResponse = {
  status: "success",
  data: { userId: "123", name: "Alice" },
