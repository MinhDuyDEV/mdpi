...
|---|---|
| "I'll do Core Data operations on the main thread" | Main-thread Core Data blocks the UI. Use background contexts for all non-UI operations. |
| "The merge policy doesn't matter for now" | Default merge policy (error) crashes on save conflicts. Set merge policy explicitly. |
| "I'll add migrations when the schema changes" | Schema changes without migrations = data loss. Write the migration with the schema change. |
| "Core Data handles thread safety" | Core Data provides the tools. You must use the right context on the right queue. |

## Red Flags

- ViewContext used for write operations (should use background context)
- Merge policy left at default (NSErrorMergePolicy crashes on conflict)
- Fetch requests without batch size on large datasets
- Core Data stack initialized on main thread synchronously
- No migration test for schema changes

## Verification

After writing Core Data code:

- [ ] Write operations use background contexts (not ViewContext)
- [ ] Merge policy is explicitly set (not left at default NSErrorMergePolicy)
- [ ] Fetch requests on large datasets use batch size and fetchLimit
- [ ] Core Data stack initialization is async on a background queue
- [ ] Schema migrations are tested with data from the previous version
