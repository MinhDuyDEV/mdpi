...
|---|---|
| "I'll test emails in production" | Broken emails in production damage brand trust. Test with Resend's test mode. |
| "The email template looks fine in my client" | Email clients render differently. Test in Gmail, Outlook, Apple Mail. |
| "I'll handle bounces later" | Unhandled bounces degrade deliverability. Set up webhook handlers from day one. |

## Red Flags

- Email sending without error handling for bounce/complaint
- Templates not tested across email clients (Gmail, Outlook, Apple Mail)
- Reply-to or from addresses not configured
- Unsubscribe links missing in marketing emails
- API key committed to version control

## Verification

After implementing Resend emails:

- [ ] Emails tested in test mode before production
- [ ] Templates render correctly in Gmail, Outlook, and Apple Mail
- [ ] Bounce and complaint webhooks are handled
- [ ] Unsubscribe links are present in all marketing emails
- [ ] API key is stored in environment variables, not committed to code
