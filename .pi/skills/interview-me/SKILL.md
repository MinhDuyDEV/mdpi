---
name: interview-me
description: "Extracts what the user actually wants through one-question-at-a-time interview until ~95% confidence. Use when the ask is underspecified ('build me X' without who/why/success), user invokes 'interview me' / 'grill me' / 'are we sure?', or when catching yourself silently filling ambiguous requirements before any plan, spec, or code exists. Risk: Low."
---
...
Agent: [starts building a dashboard, making assumptions about who, what metrics, what success]
```
...
Agent: [Continues — discovered the actual ask isn't "a dashboard" but "a list"]
```
...
- **`brainstorming`**: parallel. Both are pre-code exploration. `interview-me` extracts intent; `brainstorming` generates options once intent is clear.
