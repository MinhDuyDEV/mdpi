...
When Fallow reports a finding, the `actions[]` array in JSON output tells you exactly what can be fixed and how:
...
### Rule
- **Auto-fixable (`auto_fixable: true`)** → apply with `npx fallow fix --dry-run` first, then `npx fallow fix --yes`
- **Not auto-fixable** → investigate with `--trace FILE:EXPORT` before any deletion
- **Remediate from the lint message**: if a finding repeats across agent sessions, add a permanent rule to AGENTS.md or this skill so the pattern becomes structurally impossible to repeat (see AGENTS.md: The Ratchet Principle)

### Quick Reference

| Issue Type | Fix Command | Auto-Fixable |
|---|---|---|
| Unused exports | `fallow fix --yes` | [x] Yes |
| Unused dependencies | `fallow fix --yes` | [x] Yes |
| Unused files | Investigate with `--trace-file`, delete manually | [ ] Manual |
| Duplicate code | Extract to shared utility, update callers | [ ] Manual |
| Complexity > 20 | Refactor into smaller functions | [ ] Manual |
| Circular deps | Extract shared code to new module | [ ] Manual |
...
|---|---|
| "The codebase is small, we don't need analysis" | Small codebases accumulate dead code faster than large ones — no one's watching. |
| "I'll clean up the warnings later" | Fallow warnings accumulate silently. Fix them as they appear or they become permanent. |
| "Duplicate code is a minor issue" | Duplication multiplies maintenance cost. Every copy is a future bug vector. |
| "Circular dependencies are fine if it works" | Circular dependencies make code untestable and unrefactorable. Break the cycle. |

## Red Flags

- Fallow warnings acknowledged but never fixed
- Dead code reported but not removed ("we might need it")
- Complexity hotspots identified but refactoring deferred indefinitely
- Duplicate code detected but deduplication postponed
- Circular dependencies flagged but architecture unchanged
