...
| `mdpi upgrade` | Bring `.pi/` up to the bundled template version. Manifest-based: overwrites only user-unmodified files, preserves user edits, lists orphans. Respects a remembered `--only` subset (won't re-add excluded categories). `--check` dry-run, `--force` overwrite modified, `--prune-all` delete orphans, `--merge-settings` union-merge `settings.json:packages` (upstream + user, other keys preserved). |
